...
const updateEconomicRestaurants = async function (RestaurantId) {
  ...
  const AvgPriceMyRestaurant = Product.findOne({where: {restaurantId: RestaurantId},
    attributes: [
      [Sequelize.fn('AVG', Sequelize.col('price')), 'avgPrice']
    ]
  })
  ...
  if (AvgPriceMyRestaurant !== null && AvgPriceOtherRestaurants !==null) {
    const finalValue = AvgPriceMyRestaurant.avgPrice <AvgPriceOtherRestaurants.avgPrice
