...
const highlight = async function (req, res) {
  ...
  try {
    ...
    if (totalHighlited >= 5) {
      ...
      await Product.update({ isHighlight: false },{ where: { id: firstHighlighted.id }, transaction: t })
    }
  
    await Product.update({ isHighlight: true },{ where: { id: req.params.productId }, transaction: t })
    await t.commit()
