...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const [ocultador, setOcultador] = useState(false)
  ...
  const texto = ocultador ? 'Cancel Order' : 'New Order '
  const simbol = ocultador ? 'minus-circle' : 'plus-circle'
  const renderHeader = () => {
    return (
      <View>
...
{ loggedInUser && (
  ...
  style={({ pressed }) => ({
    backgroundColor: ocultador === true
      ? pressed
...
>
        <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
          <MaterialCommunityIcons name={simbol} color={'white'} size={20}/>
  ...
  const validationSchema = yup.object().shape({
    ...
    OrderProducts: yup.number().positive('Negative numbers not allowed')
  })
  ...
  >
    {({ handleSubmit, setFieldValue, values }) => (
      ...
      onConfirm={ () => {
        ...
        setFieldValue('restaurantId', restaurant.id)
        setConfirmador(0)
        setProductSelection({})
        setOcultador(false)
        createOrder(values)
