...
const create = async (req, res) => {
  ...
  try {
    ...
    for (const p of req.body.products) {
      ...
      orderPrice = orderPrice + (p.quantity * product.price)
    }
    if (orderPrice <= 10) {
    ...
    order.price = orderPrice + order.shippingCosts
    ...
    for (const p of req.body.products) {
      const product = await Product.findByPk(p.productId)
      await savedOrder.addProduct(product, { through: { quantity: p.quantity, unityPrice: product.price }, transaction })
    }
    await transaction.commit()
    const storedOrder = await Order.findByPk(savedOrder.id, { include: 'products' })
...
const update = async function (req, res) {
  ...
  try {
    ...
    if (orderPrice <= 10) {
      ...
      req.body.shippingCosts = 0.0
    }
    req.body.price = orderPrice + req.body.shippingCosts
    ...
    for (const p of req.body.products) {
      const product = await Product.findByPk(p.productId)
      await updatedOrder.addProduct(product, { through: { quantity: p.quantity, unityPrice: product.price }, transaction })
    }
    await transaction.commit()
    const storedOrder = await Order.findByPk(updatedOrder.id, { include: 'products' })
